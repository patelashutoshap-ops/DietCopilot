...
Instead of acting like a passive calorie tracker, DietCopilot interprets food inputs as health signals. A user can upload a meal, compare two options, or summarize a full day of eating, and the system responds with a clear risk pattern, practical guidance, and the single next best intervention.

Built as a full-stack healthcare hackathon prototype, DietCopilot is designed to answer one simple question:
...
DietCopilot reframes nutrition as a preventive-health workflow:
...
This makes DietCopilot feel less like a tracker and more like an AI decision copilot.
...
Upload a meal photo and DietCopilot returns:
...
Describe two meal options and DietCopilot:
...
Log meals across a day and DietCopilot delivers:
...
DietCopilot is not just a concept deck. It is a working full-stack prototype with:
...
DietCopilot can evolve into a preventive-health companion for:
...
DietCopilot validates model outputs against strict schemas. If the live OpenAI call fails, the backend returns a safe fallback response so the UI remains stable.
...
DietCopilot shows how AI can move nutrition from passive logging to proactive intervention.
...
>>>>>>> 5a1b287 (Initial commit for DietCopilot)
